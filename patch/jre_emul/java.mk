...
$(EMULATION_JAR): $(ALL_JAVA_SOURCES)
	@mkdir -p $(@D)
	@echo "building jre_emul.jar ${PWD}"
	printf "%s\n" $(ALL_JAVA_SOURCES) > sources.txt;
	printf "%s\n" $(ALL_JAVA_SOURCES) > /app/j2objc/jre_emul_nobase/sources.txt;
	@set -e; stage_dir=`${MKTEMP_CMD}`; \
	  cd $(J2OBJC_ROOT)/jre_emul_nobase/ && ../scripts/javac_no_deprecated_warnings.sh $(JAVAC) \
	  ...
	  -d $$stage_dir -encoding UTF-8 -source 11 -target 11 -nowarn @sources.txt; \
